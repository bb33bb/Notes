...
## 3.5. 探测安全厂商相关IP地址
通过向https://www.maxmind.com/geoip/v2.1/city/me发出认证请求，设置请求头中的HTTP Refer和User-Agent，访问https://www.maxmind.com/en/locate-my-ip-address来获取宿主系统的地址信息。获取信息封装于JSON格式文件中，包含国家、城市、或者与IP相关的组织机构等信息。如果获取到的组织机构名称与安全厂商相匹配，恶意软件将发生异常并停止运行。
## 3.6. 检测开机时间
