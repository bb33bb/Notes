...
- [6. 其它检测方法与对抗](#6-其它检测方法与对抗)
    - [6.1. 无效操作码](#61-无效操作码)
    - [6.2. 运行时间差](#62-运行时间差)
    - [6.3. 搜索物理内存中的VMware字符串](#63-搜索物理内存中的vmware字符串)
    - [6.4. 搜索特定的驱动模块(.sys)](#64-搜索特定的驱动模块sys)
    - [6.5. 检测0环的系统内核对象，痕迹很多，甚至是内核设备的符号链接也发现了特征](#65-检测0环的系统内核对象痕迹很多甚至是内核设备的符号链接也发现了特征)
    - [6.6. BIOS检测](#66-bios检测)
    - [6.7. VirtualPC](#67-virtualpc)
- [7. 集成对抗方法](#7-集成对抗方法)
    - [7.1. IDA脚本](#71-ida脚本)
...
```
## 6.3. 搜索物理内存中的VMware字符串
## 6.4. 搜索特定的驱动模块(.sys)
## 6.5. 检测0环的系统内核对象，痕迹很多，甚至是内核设备的符号链接也发现了特征
## 6.6. BIOS检测
## 6.7. VirtualPC
