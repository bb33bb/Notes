...
- [5. 线程切换](#5-线程切换)
    ...
    - [5.7. 线程切换与FS寄存器](#57-线程切换与fs寄存器)
    - [5.8. 线程的优先级](#58-线程的优先级)
        - [5.8.1. _KiReadySummary](#581-_kireadysummary)
        - [5.8.2. 如果没有就绪线程怎么办](#582-如果没有就绪线程怎么办)
...
## 5.8. 线程的优先级
Windows通过函数KiFindReadyThread来找到下一个处于就绪状态应该被调用的线程，查找方式为按照优先级别从高到低进行查找，查找到就绪线程即停止，不会继续查找。
### 5.8.1. _KiReadySummary
这是一个32位变量，每个比特位对应一个相应优先级的调度链表。当向某个调度链表中挂入或者摘除线程后，会判断调度链表是否为空（判断KiDispatcherReadyListHead中的双向链表，如果头指针和尾指针相同且等于当前地址，则为空，如果头指针和尾指针相同但是不等于当前地址，则调度链表中存在一个线程），如果是则置变量_KiReadySummary对应比特位为0，不是则置1。这样可以提高查找就绪线程的效率。多个CPU会随机寻找KiDispatcherReadyListHead指向的数组中的线程，线程可以绑定某个CPU（setThreadAffinityMask）。
### 5.8.2. 如果没有就绪线程怎么办
当CPU发现没有就绪线程的时候，会调度空闲线程（_KPCR->PrcbData->IdleThread）