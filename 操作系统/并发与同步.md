...
# 5. 可等待对象详细介绍
不同类型（_DISPATCHER_HEADER.Type不同）的可等待对象在以下两点上存在差异：
* 对应的创建函数、激活函数
* 在等待函数的等待循环中，激活条件的判断和SingalState的修改
## 5.1. 事件：Event
### 5.1.1. 创建函数：CreateEvent
* 第二个参数代表事件类型，TRUE代表事件为通知类型对象（_DISPATCHER_HEADER.Type为0），FALSE代表事件为事件同步对象（_DISPATCHER_HEADER.Type为1）
* 第三个参数代表初始化的时候信号即SingalState的状态
### 5.1.2. 激活函数：SetEvent
...
### 5.1.3. 等待循环
如果SingalState为1，当事件为通知类型对象时，会不做任何操作；当事件为事件同步对象时，会将SingalState减去1。
