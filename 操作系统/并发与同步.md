...
- [5. 可等待对象详细介绍](#5-可等待对象详细介绍)
    ...
    - [5.3. 互斥体](#53-互斥体)
        - [5.3.1. 为什么要有互斥体](#531-为什么要有互斥体)
            - [5.3.1.1. 等待对象被遗弃](#5311-等待对象被遗弃)
            - [5.3.1.2. 重入](#5312-重入)
        - [5.3.2. 创建函数：CreateMutex](#532-创建函数createmutex)
...
### 5.3.1. 为什么要有互斥体
#### 5.3.1.1. 等待对象被遗弃
* 互斥体、信号量、事件都是内核对象，都可以用来控制跨进程的线程同步控制，比如A进程的X线程和B进程的Y线程均使用同一个可等待对象Z
* 如果B进程还没来得及调用修改Z对象的SingalState的函数就突然终结，会导致Z对象被遗弃，A进程的X线程将永远等待下去
* 互斥体不会出现这种情况，系统会激活可等待对象
#### 5.3.1.2. 重入
```c
WaitForSingleObject(A)
    ...
    WaitForMultiplesObject(A)
```
如果使用事件或者信号量，程序会陷入死锁，但是互斥体不会。
### 5.3.2. 创建函数：CreateMutex
