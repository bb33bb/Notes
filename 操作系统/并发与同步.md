...
- [5. 可等待对象详细介绍](#5-可等待对象详细介绍)
    - [5.1. 事件：Event](#51-事件event)
        ...
        - [5.1.2. 激活函数：SetEvent](#512-激活函数setevent)
    ...
    - [5.2. 信号量：SEMAPHORE](#52-信号量semaphore)
        ...
        - [5.2.2. 激活函数：ReleaseSemaphore](#522-激活函数releasesemaphore)
    - [5.3. 互斥体](#53-互斥体)
        - [5.3.1. 创建函数：CreateMutex](#531-创建函数createmutex)
        - [5.3.2. 互斥体的特点](#532-互斥体的特点)
            - [5.3.2.1. 重入](#5321-重入)
            - [5.3.2.2. 对象被遗弃](#5322-对象被遗弃)
        - [5.3.3. 激活函数：ReleaseMutex](#533-激活函数releasemutex)
...
```c
MUTANT.Header.Type = 2;            //类型为2
MUTANT.Header.SignalState = bInitialOwner?0:1;          //如果属于当前线程，信号值为0，否则为1
MUTANT.OwnerThread = bInitialOwner?当前线程结构体:null;  //如果属于当前线程，为当前线程结构体指针，否则为null
MUTANT.Abandoned = 0;
MUTANT.ApcDisable = 0;
if(bInitialOwner){
    //将当前互斥体挂入到当前线程的互斥体链表
...
```
在WaitForSingleObject中，互斥体在两种情况下可以进入临界区，一是信号值为一，二是互斥体属于当前线程（OwnerThread等于CurrentThread），这里的第二种情况决定了互斥体可以重入临界区。互斥体最多可重入临界区0x80000000次。
#### 5.3.2.2. 对象被遗弃
互斥体、信号量、事件都是内核对象，都可以用来进行跨进程的线程同步控制（A进程的X线程和B进程的Y线程使用一个可等待对象Z）。若B进程意外终结，会导致Z对象被遗弃，A进程的X线程将永远等待下去。而互斥体不会出现这种情况，MmUnloadSystemImage函数会调用KeReleaseMutant来激活互斥体。
### 5.3.3. 激活函数：ReleaseMutex
...
```
### 5.3.4. 内核互斥体
内核和用户空间创建互斥体的时候，使用的函数不同。用户空间创建互斥体使用CreateMutex函数，内核使用NtCreateMutex函数。而他们的区别在于CreateMutex函数会设置ApcDisable为0，而NtCreateMutex函数会设置ApcDisable为1，这会导致禁用内核APC。禁用内核APC的操作在KeWaitForSingleObject中完成。
