...
- [4. 线程的等待与唤醒](#4-线程的等待与唤醒)
    ...
    - [4.4.3. Event](#443-event)
        - [创建Event](#创建event)
        - [SetEvent对应内核函数为KeSetEvent](#setevent对应内核函数为kesetevent)
...
```
#### 4.4.2.4. 可等待对象相关函数唤醒
其它线程调用可等待对象相关函数如SetEvent来设置可等待对象的SingalState为1的时候，会通过等待网找到所有等待该对象的线程，将他们从等待链表上摘下（并不会将线程从等待网上面摘下，所以这些线程只是半苏醒状态）。等待线程获得CPU后，仍然位于WaitForSingleObject的等待循环中，线程会判断自己是否符合激活条件，如果符合则会将自己从等待网上摘下并释放等待块占用内存，这时线程完全苏醒。如果不符合线程则会将自己再次挂上等待链表，进入等待状态。
...
## 4.4.3. Event
被等待对象主要有两点差异。
* 唤醒函数本质都是修改SignState，但是调用的函数不同。如SetEvent、ReleaseMutant。
* 判断激活条件和修改SingalState处不同
### 创建Event
`CreateEvent`这个函数用于创建一个Event，第三个参数代表初始化的时候信号即SingalState的状态。第二个参数代表事件类型，TRUE为通知类型对象（type为0），FALSE为事件同步对象（type为1）
### SetEvent对应内核函数为KeSetEvent
* 把SingalState置1
* 判断对象类型，如果类型为0，唤醒所有等待该对象的线程，如果类型为1，从链表头找到第一个唤醒
